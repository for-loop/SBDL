...
def test_transform_all_test_data_returns_eight_rows(spark):
    ...
    accounts_df = spark.read.format("csv").option("header", "true").schema(AccountsSchema.get_schema()).load("test_data/accounts/")
    parties_df = spark.read.format("csv").option("header", "true").schema(PartiesSchema.get_schema()).load("test_data/parties/")
    addresses_df = spark.read.format("csv").option("header", "true").schema(AddressesSchema.get_schema()).load("test_data/party_address/")
